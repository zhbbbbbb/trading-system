...
//enum FutureType {AGRICULTURAL, ENERGY, METAL, CURRENCY, FINANCIAL};
...
public:
  // Bond ctor
  Future(string _productId, string _ticker, float _strike, date _expirationDate);
  Future();

  // Return the ticker of the future
  string GetTicker() const;

  // Return the coupon of the future
  float GetStrike() const;

  // Return the expiration date of the future
...
protected:
  ...
  date expirationDate; // maturity date variable
};
...
{
public:
  EuroDollarFuture(string _productId, float _strike, date _expirationDate);
  EuroDollarFuture();
private:
  string productId; // product identifier variable
  float strike; // strike variable
  static string ticker; // all EuroDollar futures have ticker EURUSD, hence static
  date expirationDate; // maturity date variable
};
...
{
public:
  BondFuture(string _productId, string _ticker, float _strike, date _expirationDate);
  BondFuture();
private:
  string productId; // product identifier variable
  string ticker; // ticker variable
...
Future::Future(string _productId, string _ticker, float _strike, date _expirationDate) : Product(_productId, FUTURE), ticker(_ticker), strike(_strike), expirationDate(_expirationDate)
{
...
{
  output << future.GetTicker() << " " << future.GetStrike() << " " << future.GetExpirationDate();
  return output;
}

string EuroDollarFuture::ticker = "EURUSD";

EuroDollarFuture::EuroDollarFuture(string _productId, float _strike, date _expirationDate) : Future(_productId, EuroDollarFuture::ticker, _strike, _expirationDate)
{
...
BondFuture::BondFuture(string _productId, string _ticker, float _strike, date _expirationDate) : Future(_productId, _ticker, _strike, _expirationDate)
{
